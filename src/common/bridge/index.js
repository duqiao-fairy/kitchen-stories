...
const _clientShareButtonOpts = {
  buttonText: '',
  /**
   * 1 表示默认小图片, window.sendduqiaoJsInfo 执行分享功能
   * 0 表示自定义文字, window.sendduqiaoJsInfo 执行回调函数逻辑
   */
  buttonImageType: 1,
...
const bridge = {
  ...
  setTitleRightButton (clientShareButtonOpts) {
    const message = {
      action: types.SET_TITLE_RIGHT_BUTTON,
      data: Object.assign(_clientShareButtonOpts, clientShareButtonOpts)
    }
  ...
  rightButtonClick (info) {
    ...
    switch (response.action) {
      case types.ON_TITLE_RIGHT_BUTTON_CLICK: 
        switch (_clientShareButtonOpts.buttonImageType) {
          /**
           * 用户自定义按钮, 走自己的callback逻辑
           */
          case 0:
            _clientShareButtonOpts.callback(response)
            break
